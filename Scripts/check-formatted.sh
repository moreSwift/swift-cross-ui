...
if which java &>/dev/null; then
  if [ -z "$1" ]; then
    ...
    echo "Running ktfmt..."
    java -jar Tools/ktfmt.jar --kotlinlang-style --quiet --dry-run --set-exit-if-changed Sources/AndroidBackend/Kotlin/
  elif find "$dir/$1" -name "*.kt" | grep '^' &>/dev/null; then
    ./Scripts/ensure_ktfmt.sh
    echo "Running ktfmt..."
    java -jar Tools/ktfmt.jar --kotlinlang-style --quiet --dry-run --set-exit-if-changed "$dir/$1"
  fi
else
