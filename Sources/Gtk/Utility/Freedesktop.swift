...
public class Bus {
    ...
    public func call(namespace: String, key: String, method: String) throws(FreedesktopError
        .BusError)
        -> OpaquePointer
    {
        let v = g_variant_parse(
            nil,
            "('\(namespace)', '\(key)')",
            nil,
            nil,
        ...
        guard let v else { throw .malformedInput }
    ...
    public func call(method: String, parameters: OpaquePointer) throws(FreedesktopError.BusError)
        -> OpaquePointer
    {
        let output = g_dbus_proxy_call_sync(
            proxy,
    ...
    public func readProperty(_ name: String) -> OpaquePointer? {
        g_dbus_proxy_get_cached_property(proxy, name)
    }
...
private func requestResponse(
    bus: Bus, method: String, input: (String) -> OpaquePointer?
) throws(FreedesktopError.BusError) -> OpaquePointer {
    ...
    typealias State = (done: Bool, response: UInt32, results: OpaquePointer?)
    let state = UnsafeMutablePointer<State>.allocate(capacity: 1)
    state.initialize(to: (false, 2, nil))
    defer {
        if let results = state.pointee.results { g_variant_unref(results) }
    ...
    let callback: DBusSignalHandler = { _, _, _, _, _, params, data in
        ...
        if let child1 = walker.child(1) {
            s.pointee.results = g_variant_ref(child1.pointer)
        }
        s.pointee.done = true
    }
    ...
    let raw = try bus.call(method: method, parameters: parameters)
    defer { g_variant_unref(raw) }

    guard let actualHandle = GVariant(raw)?.child(0)?.string() else {
        g_dbus_connection_signal_unsubscribe(connection, sub)
        throw .failedCall
    }
...
public enum Freedesktop {
    public enum Portal {
        public enum Settings {
            ...
            public enum Version {
                public static func get() throws(FreedesktopError.BusError) -> UInt32 {
                    guard let prop = bus.readProperty("version"),
                        let result = GVariant(prop)?.uint32()
                    else { throw .failedCall }
                    return result
                }
            }

            fileprivate struct _SettingField<T> {
                ...
                func read() throws(FreedesktopError.BusError) -> T {
                    let raw = try bus.call(namespace: namespace, key: key, method: "Read")
                    defer { g_variant_unref(raw) }
                    guard let value = decodeResult(raw) else { throw .failedCall }
                    return value
                }
            ...
            public enum Appearance {
                public enum ColorScheme {
                    private static let _field = _SettingField(
                    ...
                    public static func observe() -> AsyncStream<UInt32> { _field.observe() }
                    public static func subscribe(_ handler: @escaping (UInt32) -> Void) -> Task<
                        Void, Never
                    > { _field.subscribe(handler) }
                }

                public enum AccentColor {
                    private static let _field = _SettingField<(Float64, Float64, Float64)>(
                        namespace: "org.freedesktop.appearance",
                        key: "accent-color",
                        decodeResult: { raw in
                    ...
                    ) -> Task<Void, Never> { _field.subscribe(handler) }
                }
        ...
        public enum Account {
            ...
            public enum UserInformation {
                private static func read(_ key: String) throws(FreedesktopError.BusError)
                    -> String
                {
                    let raw = try requestResponse(
                        bus: bus,
                        method: "GetUserInformation"
                    ) { token in
                        g_variant_parse(
                            nil, "('', @a{sv} {'handle_token': <'\(token)'>})", nil, nil, nil)
                    }
                    defer { g_variant_unref(raw) }
                    guard let value = GVariant(raw)?.lookup(key)?.string() else {
                        throw .failedCall
                    }
                    return value
                }
        ...
        public enum Location {
            ...
            private static var _bus: Bus?
            private static var _sessionHandle: String?
            private static var _started = false
            private static let _lock = NSRecursiveLock()

            private static func _getBus() throws(FreedesktopError.BusError) -> Bus {
                if let bus = _bus { return bus }
                _lock.lock()
                defer { _lock.unlock() }
                if let bus = _bus { return bus }
                let bus = try Bus(interfaceName: "Location")
                _bus = bus
                return bus
            }

            private static func _startSession(options: Options)
                throws(FreedesktopError.BusError)
            {
                ...
                guard
                    let input = g_variant_parse(
                        nil, "(@a{sv} {\(parts.joined(separator: ", "))},)", nil, nil, nil)
                else { throw .malformedInput }
                defer { g_variant_unref(input) }
                let result = try bus.call(method: "CreateSession", parameters: input)
                defer { g_variant_unref(result) }
                guard let sessionHandle = GVariant(result)?.child(0)?.string()
                else { throw .failedCall }
                _sessionHandle = sessionHandle

                let parent = options.parentWindow.isEmpty ? "''" : "'\(options.parentWindow)'"
                let response = try requestResponse(bus: bus, method: "Start") { token in
                    g_variant_parse(
                        nil,
            ...
            fileprivate static func _readFull(
                options: Options = .init()
            ) throws(FreedesktopError.BusError) -> LocationData {
                try _startSession(options: options)
                _lock.lock()
                let sessionHandle = _sessionHandle!
                _lock.unlock()
                let bus = try _getBus()
                guard let connection = bus.connection() else { throw .failedCall }
            ...
            fileprivate static func _observeFull(
                options: Options = .init()
                ...
                AsyncStream { continuation in
                    ...
                    do {
                        try _startSession(options: options)
                        _lock.lock()
                        guard let sh = _sessionHandle else {
                            _lock.unlock()
                            continuation.finish()
                            return
                        }
            ...
            public enum Latitude {
                private static let _field = _LocationField(keyPath: \.latitude)
                public static func read(options: Options = .init()) throws(FreedesktopError
                    .BusError) -> Double?
                { try _field.read(options: options) }
                public static func observe(options: Options = .init()) -> AsyncStream<Double?> {
                    _field.observe(options: options)
                }
                public static func subscribe(
                    options: Options = .init(),
                    _ handler: @escaping (Double?) -> Void
                ) -> Task<Void, Never> { _field.subscribe(options: options, handler) }
            }

            public enum Longitude {
                private static let _field = _LocationField(keyPath: \.longitude)
                public static func read(options: Options = .init()) throws(FreedesktopError
                    .BusError) -> Double?
                { try _field.read(options: options) }
                public static func observe(options: Options = .init()) -> AsyncStream<Double?> {
            ...
            public enum Altitude {
                private static let _field = _LocationField(keyPath: \.altitude)
                public static func read(options: Options = .init()) throws(FreedesktopError
                    .BusError) -> Double?
                { try _field.read(options: options) }
                public static func observe(options: Options = .init()) -> AsyncStream<Double?> {
            ...
            public enum Accuracy {
                private static let _field = _LocationField(keyPath: \.accuracy)
                public static func read(options: Options = .init()) throws(FreedesktopError
                    .BusError) -> Double?
                { try _field.read(options: options) }
                public static func observe(options: Options = .init()) -> AsyncStream<Double?> {
            ...
            public enum Speed {
                private static let _field = _LocationField(keyPath: \.speed)
                public static func read(options: Options = .init()) throws(FreedesktopError
                    .BusError) -> Double?
                { try _field.read(options: options) }
                public static func observe(options: Options = .init()) -> AsyncStream<Double?> {
            ...
            public enum Heading {
                private static let _field = _LocationField(keyPath: \.heading)
                public static func read(options: Options = .init()) throws(FreedesktopError
                    .BusError) -> Double?
                { try _field.read(options: options) }
                public static func observe(options: Options = .init()) -> AsyncStream<Double?> {
            ...
            public enum TimestampSeconds {
                private static let _field = _LocationField(keyPath: \.timestampSeconds)
                public static func read(options: Options = .init()) throws(FreedesktopError
                    .BusError) -> Int?
                { try _field.read(options: options) }
            ...
            public enum TimestampMicroseconds {
                private static let _field = _LocationField(keyPath: \.timestampMicroseconds)
                public static func read(options: Options = .init()) throws(FreedesktopError
                    .BusError) -> Int?
                { try _field.read(options: options) }
                public static func observe(options: Options = .init()) -> AsyncStream<Int?> {
